<template>
  <section
    class="text-white flex flex-col relative justify-center items-start bg-transparent h-[80vh] gap-[10px] grow px-[3%] sm:px-[10%]"
    id="home">
    <div class="flex flex-row">
      <div class="intro-item basis-full">
        <h1 class="relative text-5xl sm:text-6xl md:text-9xl text-black dark:text-white font-extralight" ref="intro">
          Hello,<br />I'm
          <span class="heading-text font-bold">Eoin Fehily.</span>
        </h1>
        <h3 id="subIntro"
          class="text-black dark:text-white opacity-0 sub-text text-xl after:content-[''] after:bg-white after:absolute after:top-0 after:bottom-0 after:w-0.5 after:right-0">
          &nbsp;{{ typedText }}
        </h3>
      </div>
    </div>
    ...
    <div class="flex flex-row opacity-0" id="button">
      <a href="../assets/cv.pdf" download>
        <button
          ...
          <svg class="z-[2] text-color" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            viewBox="0 0 16 16">
            <path
...
export default {
  ...
  mounted() {
    ...
    timeline
      .from(this.$refs.intro, { y: -60, opacity: 0, duration: 1 })
      .to("#subIntro", {
        opacity: 1,
        duration: 0.5,
